...
func doScanList(
	...
	if bindToAttrName == "" {
		return errors.NewCode(codes.CodeInvalidParameter, `bindToAttrName should not be empty`)
	}
	...
	for i := 0; i < arrayValue.Len(); i++ {
		...
		if len(relationDataMap) > 0 && !relationFromAttrValue.IsValid() {
			return errors.NewCodef(codes.CodeInvalidParameter, `invalid relation fields specified: "%v"`, relationFields)
		}
		...
		if relationFields != "" && !relationBindToFieldNameChecked {
			...
			if !relationFromAttrField.IsValid() {
				var (
					fieldMap, _ = structs.FieldMap(structs.FieldMapInput{
		...
		default:
			return errors.NewCodef(codes.CodeInvalidParameter, `unsupported attribute type: %s`, bindToAttrKind.String())
		}
	}
